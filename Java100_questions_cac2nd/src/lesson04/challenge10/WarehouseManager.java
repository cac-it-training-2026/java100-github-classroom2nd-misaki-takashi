...
public class WarehouseManager {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < bagC.length; i++) {
			if (Math.random() < 0.25) {
				bagC[i] = 0;
			} else {
				bagC[i] = (int) (Math.random() * 10 + 1);
			}
		}

		for (int i = 0; i < bagD.length; i++) {
			if (Math.random() < 0.25) {
				bagD[i] = 0;
			} else {
				bagD[i] = (int) (Math.random() * 10 + 1);
			}
		}
